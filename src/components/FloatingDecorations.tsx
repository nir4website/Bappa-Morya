...
export const FloatingDecorations: React.FC<FloatingDecorationsProps> = ({ onShowToast }) => {
  ...
  const handleBellRing = (side: 'left' | 'right') => {
    ...
    if (onShowToast) {
      onShowToast(`🔔 Ghantinaad (घंटीनाद) — ${side === 'left' ? 'Jai Ganesh' : 'Mangal Moorti Morya'}!`);
    }
  };
  ...
  return (
    <div className="pointer-events-none fixed inset-0 z-20 overflow-hidden" aria-hidden="true">
      ...
      <div className="absolute top-[26%] right-[10%] animate-slow-float opacity-75 hidden md:block">
        <RedHibiscus size={28} />
      </div>
      <div className="absolute top-[65%] left-[8%] animate-slow-float-rev opacity-70 hidden md:block">
        <RedHibiscus size={26} />
      </div>
